        ...
        BlocBuilder<UserCubit, UserState>(
          builder: (context, state) {
            return SubscriptionPlanItem(
              ...
              buttonText: (state.isSubscribed && state.planLookUpKey == AgencyPlan.new_agency_starter.name) ? TempLanguage().lblCancel : TempLanguage().lblSubscribe,
              onTap: () async {
                try {
                  ownShowConfirmDialogCustom(
                      context,
                      title: (state.isSubscribed && state.planLookUpKey == AgencyPlan.new_agency_starter.name)
                          ? TempLanguage().lblCancelSubscription
                          : TempLanguage().lblSubscribe,
                      subTitle: (state.isSubscribed && state.planLookUpKey == AgencyPlan.new_agency_starter.name)
                      ...
                      onAccept: (ctx) {
                        ...
                        if (state.isSubscribed) {
                          if (state.planLookUpKey == AgencyPlan.new_agency_starter.name) {
                            ...
                            cancelSubscription(userCubit, userService, state.subscriptionId, AgencyPlan.new_agency_starter.name, context);
                          } else {
                            showLoadingDialog(context, null);
                            updateSubscription(userCubit, userService, state.subscriptionId, AgencyPlan.new_agency_starter.name, context);
                          }
                        } else {
                          showLoadingDialog(context, null);
        ...
        BlocBuilder<UserCubit, UserState>(
          builder: (context, state) {
            return SubscriptionPlanItem(
              ...
              buttonText: (state.isSubscribed && state.planLookUpKey == AgencyPlan.new_agency_growth.name) ? TempLanguage().lblCancel : TempLanguage().lblSubscribe,
              onTap: () async {
                try {
                  ownShowConfirmDialogCustom(
                      context,
                      title: (state.isSubscribed && state.planLookUpKey == AgencyPlan.new_agency_growth.name)
                          ? TempLanguage().lblCancelSubscription
                          : TempLanguage().lblSubscribe,
                      subTitle: (state.isSubscribed && state.planLookUpKey == AgencyPlan.new_agency_growth.name)
                          ? TempLanguage().lblCancelSubscriptionInfo
                          : TempLanguage().lblSubscribeInfo,
                      positiveText: TempLanguage().lblOk,
                      cancelable: false,
                      ...
                      onAccept: (ctx) {
                        ...
                        if (state.isSubscribed) {
                          if (state.planLookUpKey == AgencyPlan.new_agency_growth.name) {
                            showLoadingDialog(context, null);
                            cancelSubscription(userCubit, userService, state.subscriptionId, AgencyPlan.new_agency_growth.name, context);
                          } else {
                            showLoadingDialog(context, null);
                            updateSubscription(userCubit, userService, state.subscriptionId, AgencyPlan.new_agency_growth.name, context);
                          }
                        } else {
                          showLoadingDialog(context, null);
        ...
        BlocBuilder<UserCubit, UserState>(
          builder: (context, state) {
            return SubscriptionPlanItem(
              ...
              buttonText: (state.isSubscribed && state.planLookUpKey == AgencyPlan.establish_agency_starter.name) ? TempLanguage().lblCancel : TempLanguage().lblSubscribe,
              onTap: () async {
                try {
                  ownShowConfirmDialogCustom(
                      context,
                      title: (state.isSubscribed && state.planLookUpKey == AgencyPlan.establish_agency_starter.name)
                          ? TempLanguage().lblCancelSubscription
                          : TempLanguage().lblSubscribe,
                      subTitle: (state.isSubscribed && state.planLookUpKey == AgencyPlan.establish_agency_starter.name)
                          ? TempLanguage().lblCancelSubscriptionInfo
                          : TempLanguage().lblSubscribeInfo,
                      positiveText: TempLanguage().lblOk,
                      cancelable: false,
                      ...
                      onAccept: (ctx) {
                        ...
                        if (state.isSubscribed) {
                          if (state.planLookUpKey == AgencyPlan.establish_agency_starter.name) {
                            showLoadingDialog(context, null);
                            cancelSubscription(userCubit, userService, state.subscriptionId, AgencyPlan.establish_agency_starter.name, context);
                          } else {
                            showLoadingDialog(context, null);
                            updateSubscription(userCubit, userService, state.subscriptionId, AgencyPlan.establish_agency_starter.name, context);
                          }
                        } else {
                          showLoadingDialog(context, null);
        ...
        BlocBuilder<UserCubit, UserState>(
          builder: (context, state) {
            return SubscriptionPlanItem(
              ...
              buttonText: (state.isSubscribed && state.planLookUpKey == AgencyPlan.establish_agency_growth.name) ? TempLanguage().lblCancel : TempLanguage().lblSubscribe,
              onTap: () async {
                try {
                  ownShowConfirmDialogCustom(
                      context,
                      title: (state.isSubscribed && state.planLookUpKey == AgencyPlan.establish_agency_growth.name)
                          ? TempLanguage().lblCancelSubscription
                          : TempLanguage().lblSubscribe,
                      subTitle: (state.isSubscribed && state.planLookUpKey == AgencyPlan.establish_agency_growth.name)
                          ? TempLanguage().lblCancelSubscriptionInfo
                          : TempLanguage().lblSubscribeInfo,
                      positiveText: TempLanguage().lblOk,
                      cancelable: false,
                      ...
                      onAccept: (ctx) {
                        ...
                        if (state.isSubscribed) {
                          if (state.planLookUpKey == AgencyPlan.establish_agency_growth.name) {
                            showLoadingDialog(context, null);
                            cancelSubscription(userCubit, userService, state.subscriptionId, AgencyPlan.establish_agency_growth.name, context);
                          } else {
                            showLoadingDialog(context, null);
                            updateSubscription(userCubit, userService, state.subscriptionId, AgencyPlan.establish_agency_growth.name, context);
                          }
                        } else {
                          showLoadingDialog(context, null);
